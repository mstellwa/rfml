...
#'  # using field filter
#'  df <- ml.data.frame("försäkringskassan AND kalle", fieldFilter="parti==m, replik==Y")
#' }
#' @export
...
#' for each row. Document URI is generated by MarkLogic and the directory is named based on the string rfml,
...
as.ml.data.frame <- function (x, name, format = "json", directory = "") {
  ...
  if (!is.data.frame(x)) {
    stop("Only objects of data.frame type are supported!")
  }
...
#' Save the data of a ml.data.frame object data to MarkLogic.
#'
#' The function will create documents based on the definition of a ml.data.frame object.
#' It will create a new document for each result from the serach that the ml.data.frame
#' is based on. Document URI is generated by MarkLogic and the directory is named based
#' on the string rfml,the username and the name parameter, for example /rfml/admin/iris/.
#' The documents will belong to a collection named "name".
#'
#' @param x a ml.data.frame object.
#' @param name The name of the object. This will be used to name the collection that the data belongs to.
#' @param format The format od the documents that is created, json or XML. Default is json
#' @param directory The directory to save the documents, needs to start and end with a /. Default saved to /rfml/[username]/[name]/
#' @return A ml.data.frame object.
#' @examples
...
#' }
#' @export
save.ml.data.frame <- function(x, name, format = "json", directory = "" ){
  if (!is.ml.data.frame(x)) {
    stop("Only objects of ml.data.frame type are supported!")
  }
  rfmlCollection <- "function!"
  # create a ml.data.frame object based on a collection search
  return(ml.data.frame(collection=c(rfmlCollection)));
}
################ [ ############################
