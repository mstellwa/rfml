...
## Using rfml
Before data can be selected a call to ml.connect is needed, the function verifies that the database is setup correctly and also saves the connection information to be used by other functions.
```R
#create a connection
...
```
Data can also be selected using simple field level filtering. Currently only == is supported.
```R
